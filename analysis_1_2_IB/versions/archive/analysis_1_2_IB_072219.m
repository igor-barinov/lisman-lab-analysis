...
function varargout = analysis_1_2_IB_072219(varargin)
% ANALYSIS_1_2_IB_072219 MATLAB code for analysis_1_2_IB_072219.fig
%      ANALYSIS_1_2_IB_072219, by itself, creates a new ANALYSIS_1_2_IB_072219 or raises the existing
%      singleton*.
%
%      H = ANALYSIS_1_2_IB_072219 returns the handle to a new ANALYSIS_1_2_IB_072219 or the handle to
%      the existing singleton*.
%
%      ANALYSIS_1_2_IB_072219('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANALYSIS_1_2_IB_072219.M with the given input arguments.
%
%      ANALYSIS_1_2_IB_072219('Property','Value',...) creates a new ANALYSIS_1_2_IB_072219 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before analysis_1_2_IB_072219_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to analysis_1_2_IB_072219_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help analysis_1_2_IB_072219

% Last Modified by GUIDE v2.5 10-Jan-2020 13:01:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @analysis_1_2_IB_072219_OpeningFcn, ...
                   'gui_OutputFcn',  @analysis_1_2_IB_072219_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function analysis_1_2_IB_072219_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to analysis_1_2_IB_072219 (see VARARGIN)

% Choose default command line output for analysis_1_2_IB_072219
...
function varargout = analysis_1_2_IB_072219_OutputFcn(hObject, eventdata, handles) 
...
if isempty(fileType) % No data loaded
    ...
    set(handles.solutionTable, 'Data', {});
...
elseif strcmp(fileType, 'avg')
    ...
    uimenu(handles.menuToggleROI, 'Label', 'Selected', 'Callback', {@menuToggleSelectedROI_Callback, handles});
    colLabels = {};
    for i = 1:numROI
...
if iscell(info)
    ...
    [solTiming, indices] = sort(solTiming);
...
hdl = gca;
function plot_annotations(axis, results)
    xlim = get(axis, 'xlim');
    ...
    try
        ...
        annotation('textbox', [x1 y x2-x1 y1-y], 'String', results.solA, 'linestyle', 'none');
        annotation('textbox', [x2 y x3-x2 y1-y], 'String', results.solB, 'linestyle', 'none');
        annotation('textbox', [x1 y2 x2-x1 y3-y2], 'String', results.solC, 'linestyle', 'none');
        annotation('textbox', [x2 y2 x3-x2 y3-y2], 'String', results.solD, 'linestyle', 'none');
    catch
...
% hObject    handle to menuToggleSelectedROI (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global currentData;
global tableSelection;
...
if (isempty(tableSelection))
    ...
    for i = 1:length(rois)
        for m = 1:length(menuItems)
            ...
            if (strcmp(menuLbl, strcat('ROI', num2str(rois(i)))))
                if strcmp(get(menuItems(m), 'Checked'), 'on')
...
numBase = str2double(get(handles.inputNumBase,'String'));
washStart = str2double(get(handles.inputWashStart,'String'));

if strcmp(get(hObject, 'Checked'), 'on')
    set(hObject, 'Checked', 'off');
else
    if isnan(numBase) || isnan(washStart)
        warndlg('Please set the number of baeline points and start of washout to see annotaions');
    else
        set(hObject, 'Checked', 'on');
    end
end
