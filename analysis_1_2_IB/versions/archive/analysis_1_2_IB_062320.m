...
%
%      ANALYSIS_1_2_IB_062320, by itself, creates a new ANALYSIS_1_2_IB_062320 or raises the existing
%      singleton*.
%
%      H = ANALYSIS_1_2_IB_062320 returns the handle to a new ANALYSIS_1_2_IB_062320 or the handle to
%      the existing singleton*.
%
%      ANALYSIS_1_2_IB_062320('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANALYSIS_1_2_IB_062320.M with the given input arguments.
%
%      ANALYSIS_1_2_IB_062320('Property','Value',...) creates a new ANALYSIS_1_2_IB_062320 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before analysis_1_2_IB_062320_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to analysis_1_2_IB_062320_OpeningFcn via varargin.
%
...
%
function varargout = analysis_1_2_IB_062320(varargin)
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @analysis_1_2_IB_062320_OpeningFcn, ...
                   'gui_OutputFcn',  @analysis_1_2_IB_062320_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% varargin   command line arguments to analysis_1_2_IB_062320 (see VARARGIN)
%
function analysis_1_2_IB_062320_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for analysis_1_2_IB_062320
...
% handles    structure with handles and user data (see GUIDATA)
%
function varargout = analysis_1_2_IB_062320_OutputFcn(hObject, eventdata, handles) 
...
try
    scriptFilePath = which('analysis_1_2_IB_062320');
    [path, ~, ~] = fileparts(scriptFilePath);
    logFile = 'analysis_1_2_IB_062320_LOG.txt';
...
set(handles.('menuShowLifetime'), 'Checked', 'off');
set(handles.('menuShowGreen'), 'Checked', 'off');
set(handles.('menuShowRed'), 'Checked', 'off');
set(handles.('menuShowAnnots'), 'Checked', 'off');
...
if time_is_adjusted(handles)
    ...
    time = tableData.adjusted_time(numBasePts);
else
    time = tableData.time();
end
...
if values_are_normalized(handles)
    solutions = get_solution_info(handles);
    numBasePts = ROIUtils.number_of_baseline_pts(solutions);
    lifetime = tableData.normalized_lifetime(numBasePts);
    int = tableData.normalized_green(numBasePts);
    red = tableData.normalized_red(numBasePts);
else
...
set(handles.('dataTable'), 'ColumnName', tableData.roi_labels());
...
try
    ...
    if all(isnan(roiData.lifetime()))
        warndlg('This file has no lifetime data to plot');
        return;
    else
        toggle_menu(hObject);
    end
