...
name_start=findstr(spc.filename, filesep);
...
if exist([fname, '.mat'])
    ...
    try
        evalc(['a = ', fname, '(gui.spc.proChannel).roiData']);
    catch
        a = 0;
    end
    try
        evalc(['bg = ', fname, '(gui.spc.proChannel).bgData']);
    catch
        bg = 0;
    end
    try
        evalc(['la = ', fname, '(gui.spc.proChannel).polyLines']);
    catch
        la = 0;
    end
end
    ...
    if nRoi > 0 || length(bg.position) >= fn
        for i = 0:nRoi
            ...
            elseif siz(2) == 2
                ...
                xi = spc_roi(:,1);
                yi = spc_roi(:,2);
                gui.spc.figure.roiA(i+1) = line(xi, yi, 'Tag', rectstr, 'color', 'cyan', 'ButtonDownFcn', 'spc_dragRoiB'); % nicko changed spc_dragRoiA to spc_dragRoiB   to direct to correct routine
                gui.spc.figure.textA(i+1) = text(xi(1)-2, yi(1)-2, num2str(i), 'color', 'white', 'Tag', textstr, 'ButtonDownFcn', 'spc_deleteRoiA'); 
                figure(gui.spc.figure.lifetimeMap);
                axes(gui.spc.figure.lifetimeMapAxes);
                gui.spc.figure.roiB(i+1) = line(xi, yi, 'Tag', rectstr, 'color', 'cyan', 'ButtonDownFcn', 'spc_dragRoiB');  % nicko changed spc_dragRoiA to spc_dragRoiB   to direct to correct routine
                gui.spc.figure.textB(i+1) = text(xi(1)-2, yi(1)-2, num2str(i), 'color', 'white', 'Tag', textstr, 'ButtonDownFcn', 'spc_deleteRoiA');  
                if spc.switches.redImg
                    figure(gui.spc.figure.scanImgF);
                    axes(gui.spc.figure.scanImgA);
                    gui.spc.figure.roiC(i+1) = line(xi, yi, 'Tag', rectstr, 'color', 'cyan', 'ButtonDownFcn', 'spc_dragRoiB');  % nicko changed spc_dragRoiA to spc_dragRoiB   to direct to correct routine
