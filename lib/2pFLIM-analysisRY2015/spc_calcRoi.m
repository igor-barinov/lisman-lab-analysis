...
nChannels = spc.SPCdata.scan_rout_x;
                ...
                if goodRoi
                    ...
                    if roiN == 0 || isempty (roiN) %%%%%Background
                        ...
                        if spc.switches.noSPC
                            ...
                            stst=[spc.datainfo.date, ' ', spc.datainfo.time];
                            a(roiN).time(fn) = datenum([spc.datainfo.date, ' ', spc.datainfo.time]);
                            a(roiN).time3(fn) = datenum(spc.datainfo.triggerTime);
                            a(roiN).fraction2(fn) = spc_getFraction((tau_m + spc.fit(gui.spc.proChannel).t_offset)*1000/spc.datainfo.psPerUnit); %tauD*(tauD-tau_m)/(tauD-tauAD) / (tauD + tauAD -tau_m);
                            a(roiN).tauD(fn) = tauD;
                        ...
                        if spc.switches.redImg
                            ...
                            a(roiN).ratio(fn) = a(roiN).green_mean(fn)./a(roiN).red_mean(fn);
