...
for j = 1:nRoi
    if ishandle(gui.spc.figure.roiB(j));
        ...
        if goodRoi
           ...
           if j>1,
               F_int = F_int(find(F_int>avg_bg_intensity+3));
               %figure;
               %subplot(2,2,1); image(project1);
               newimg = project1*0;
               newimg(ROIreg) = 255;
               %subplot(2,2,2); image(newimg);
               newimg2 = project1 * 0;
               %subset_ROIreg_ind =
               mypixels=intersect(find(ROIreg),find(project1>avg_bg_intensity+3));
               newimg2(mypixels) = 255;
               %subplot(2,2,3); image(newimg2);
           end;
%             
            intensity = sum(F_int(:));
...
if ~spc.switches.noSPC
    fig_content = {'tau_m', 'int_int2', 'red_int2'};
    fig_yTitle = {'Lifetime', 'Intensity(Green)', 'Intensity(Red)'};
else
...
for subP = 1:3  %Three figures
    ...
    if subP ==1 
            hold on;
            plot(t, aveval, '-ok', 'LineWidth', 1.5);     %plots average, only for first graph (lifetime)  JZ
    end
