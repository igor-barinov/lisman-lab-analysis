...
for j = 1:nRoi
    if ishandle(gui.spc.figure.roiB(j));
        ...
        spc_avg_bg =spc.switches.lutlim(1) %*avg_bg_intensity;
        spc_ab_std = 1%*ab_std;
        ...
        if goodRoi
            ...
            if j ~= 1
                if ~spc.switches.noSPC
                    bw = (spc.project >= spc.switches.lutlim(1));   %low threshold for lifetime image       
                    lifetimeMap = spc.lifetimeMap(bw & ROIreg);     %map of lifetime
...
figure(7);newmat = project1; newmat(newmat<spc_avg_bg+spc_ab_std)=0; 
subplot(2,1,1);image(newmat);
newroi(roipixels) = 255;         
subplot(2,1,2); image(newroi);
cmap = colormap;
newmap = cmap;
newmap(1,:) = [0 0 0];
colormap(newmap);

figure(8); image(project1); alphamask(ROIreg,[[1 1 1],0.01]);
...
if ~spc.switches.noSPC
    ...
    fig_yTitle = {'Lifetime', 'Mean Intensity', 'Mean(Red)'};
else
