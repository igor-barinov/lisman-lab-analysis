...
%             
    ...
    %                 end
            ...
            if  j == nRoi                           % if j is the last ROI
                ana.nPixels = [];                       
                ana.nPixelsum = []; 
                for i = 1:nRoi-2                        % because nRoi include background bg, subtract 1, then subtract 1 again so it does not add the final roi (which is around the dendrite)
                    ana.nPixels(i,:) = a(i).nPixel;     
                end
                nPixelsum = sum(ana.nPixels);
                ana.nPixels = a(nRoi - 1).nPixel - nPixelsum;               
            end
