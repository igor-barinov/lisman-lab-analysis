...
if armed & measuring
        ...
        pageNumber=0;
        out1=calllib('spcm32','SPC_pause_measurement',state.spc.acq.module);
        if out1 < 0
        ...
        [out1 state.spc.acq.mData]=calllib('spcm32','SPC_read_data_block',state.spc.acq.module,0,pageNumber,1,0,state.spc.acq.SPCMemConfig.block_length-1,tempData);
        if out1 < 0
            error = FLIM_get_error_string (out1);    
            disp(['Error during read data block:', error]);
            return;
        end
        out1 = calllib('spcm32','SPC_restart_measurement',state.spc.acq.module);
        if out1 < 0
            error = FLIM_get_error_string (out1);    
