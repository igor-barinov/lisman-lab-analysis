...
%      singleton*.
%
%      H = YPHYS_STIMSCOPE returns the handle to a new YPHYS_STIMSCOPE or the handle to
...
% Last Modified by GUIDE v2.5 17-Jun-2005 12:51:51
...
if strcmp(get(handles.start, 'String'), 'Start')
	...
	if ntrain < 2
        ...
        %tic;
        yphys_getGain;
        state.yphys.timer.stim_timer =timer('TimerFcn','yphys_stimLoopFcn','ExecutionMode','fixedSpacing','Period', interval);
...
set(yphys.figure.avePlot, 'XData', yphys.data.data(:,1), 'YData', yphys.data.data(:,2), 'color', 'red');
yphys_updateAverage;
