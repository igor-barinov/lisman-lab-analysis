...
figure (gui.spc.online); % plot full frame data ; NickO
    subplot(2,3,3); % fig fraction (tau2):fitting ; 
   ...
   xlabel ('Time (min)');
    ...
    subplot(2,3,[1,2]); % fig fraction (tau2
    fl1 = 0;
...
subplot(2,3,[4,5]);    % mean photon emission time 
    fl1 = 0;
    for pos = 0:max(a.position(:))
        ptime = (a.time(a.position == pos)- min(a.time))*60*24 ;
    ...
    hold off;
ylabel('mean photon emission time (tau_m2'); %nickO
xlabel ('Time (min)');
