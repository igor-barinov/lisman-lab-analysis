           ...
           if ishandle(gui.spc.figure.roiB(j));
                ...
                if j == 1  % BACKGROUND ROI!!!! 
                    if ~spc.switches.noSPC  %BACKGROUND FOR SPC ROI!!!!to find cell borders 
                        ...
                        spc_bg =BMF*(spc_bg_mean)+SD_BMF*spc_bg_std; 
                        if BMF == 0
        ...
        %                 end
               ...
               if roiN == 0 || isempty(roiN) %== [] %LOADING DATA into bg. array!! nicko
                    ...
                    if spc.switches.redImg    %%%%% RED and GREEN   ROI loading to a. 
                        ...
                        a(roiN).ratio(fn) = a(roiN).green_mean(fn)./a(roiN).red_mean(fn); % green/red ratio
...
%                 Ch(channelN).bgData = bg;
        ...
        % these values now display in thresholds.fig - AZ
          ...
          if j == 2
            if ~spc.switches.noSPC
                mTextBox4 = uicontrol(thresholds, 'style','edit','Position', [190 155 60 20]);
                set(mTextBox4,'string',spine_spc_mean/sdf); 
            else
                mTextBox5 = uicontrol(thresholds, 'style','edit', 'Position', [190 60 60 20]);
                %set(mTextBox5,'string',spine_red_mean/rdf -red_bg);
                set(mTextBox5,'string',spine_red_mean -red_bg);
                mTextBox4 = uicontrol(thresholds, 'style','edit','Position', [190 155 60 20]);
                %set(mTextBox4,'string',spine_green_mean/rdf - green_bg); 
                set(mTextBox4,'string',spine_green_mean - green_bg); 
            end
         end         
         if ~spc.switches.noSPC
            mTextBox = uicontrol(thresholds, 'style','edit','Position', [190 10 60 20]);
            set(mTextBox,'string',spc_bg_mean);  %+2*spc_ab_std
            mTextBox1 = uicontrol(thresholds, 'style','edit', 'Position', [190 35 60 20]);
            set(mTextBox1,'string',spc_bg);  %spc_bg mean +2*spc_ab_std
          else
            mTextBox2 = uicontrol(thresholds, 'style','edit','Position', [190 105 60 20]);
            set(mTextBox2,'string',(red_bg_mean + SD_BMFr*red_bg_std));
            mTextBox3 = uicontrol(thresholds, 'style','edit', 'Position', [190 130 60 20]);
...
%                 basetime = min(time1);
%             end
%             ana.LocTime = (time1 - basetime)*24*60;
%         end   
        ana.green_mean = [];
        ana.red_mean = [];
        ana.SpDendRatio = [];
...
%          end
        ...
        if ~spc.switches.noSPC
            ...
            ana.ave_spc_mean = [];
            ana.spc_sum = [];
            ana.ave_spc_sum = [];   %nicko - added in to calculate average spc_sum int for all ROIs
            for i = 1:numROI
                %ana.time3(i,:) = a(i).time3;
                ana.time(i,:) = a(i).time;
                ...
                ana.ave_spc_sum = nanmean(ana.spc_sum);%NickO - added in to calculate integral spc_sum int for all ROIs
                ana.red_sum(i,:)= a(i).red_sum;
                ana.red_mean(i,:)= a(i).red_mean;
            end
            ...
            for i = 1:numROI-1 %nicko    calcultaion os Sp to Dendr ratio G/R normalised 
                 ...
                 i=1;
                ana.nickdata(i,:) = ana.time(i,:);
                %ana.time(i,:) = a(i).time;
                ana.nickdata(i+1,:) = ana.green_mean(i,:);
                ana.nickdata(i+2,:) = ana.green_mean(i+1,:);
                ana.nickdata(i+3,:) = ana.red_mean(i,:);
                 ana.nickdata(i+4,:) = ana.red_mean(i+1,:);
                ana.nickdata(i+5,:) = ana.SpDendRatio(i,:);
                ana.nickdata(i+6,:) = ana.SpDendRatio(i+1,:);
%                 ana.green_mean(i,:)= a(i).green_mean;
%                 ana.red_mean(i,:)= a(i).red_mean;
%                 ana.SpDendRatio(i,:) = ana(i).SpDendRatio;
...
%            else
               ...
               %evalc(['val = Ch(channelN).roiData(j).',%fig_content{subP}]);%RY
