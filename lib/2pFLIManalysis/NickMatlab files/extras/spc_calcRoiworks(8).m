...
filterwindow = 3; %seting filter. if =3 makes 3x3 smoothing before calculation-nick
BMF =0; %backgraund multiplication factor: spc_background = BMF*mean+2*std; if BMF=0 then background = low threthould LUT in lifetime map fig3-nick
BMFr=1;% bg=BMFr * mean+2std similar to above but if BMF =0 then red_background = set_red_bg
SD_BMFr=1.5
; % muliplication factor for SD . nomally it is =2
set_red_bg = 0;% if 0 then bg = mean + 2 SD- nicko
% pixelshift=0;
sdf =4;    % this and nex parameteres are only to calculate suggested backgraund displaied in the dialog box -nicko
rdf=1.6;
Red =0.5; Green =0; Yellow=0.5; % red image mask  color
deltaX=4; %these are ROI shift corection for the red ROI relatively spc-Roi-nicko
deltaY=0;
set_nRoi=0;% if 0 nRoi are automatic otherwise set nRoi including bg ROi
plotave =0;%-nicko 
page3 = 1;%-nicko
dpixels=0;%   Make dpixels=1 if dendpixels are calculated in the last region-nicko
...
%                     bg_green_nPixel = bg.green_int(fn) / bg.mean_int(1);             
                ...
                red_bg = BMFr*(red_bg_mean+SD_BMFr*red_bg_std); %NickO:this background is to find cell borders in spc and FLIM image data but not to subtruct from the signal
...
%                      end
                ...
                figure (14); 
                mTextBox = uicontrol('style','text','Position', [20 20 60 20]);
                set(mTextBox,'string',spc_bg_mean);  %+2*spc_ab_std
                mTextBox1 = uicontrol('style','text', 'Position', [20 70 60 20]);
                set(mTextBox1,'string',spc_bg);  %spc_bg mean +2*spc_ab_std
                mTextBox2 = uicontrol('style','text','Position', [110 20 60 20]);
                set(mTextBox2,'string',red_bg_mean); 
                mTextBox3 = uicontrol('style','text', 'Position', [110 70 60 20]);
                set(mTextBox3,'string',red_bg); %red_bg_mean +2*red_ab_std
               if j==nRoi
                mTextBox4 = uicontrol('style','text','Position', [20 120 60 20]);
                set(mTextBox4,'string',spine_spc_max/sdf); %+2*red_ab_std
                mTextBox5 = uicontrol('style','text', 'Position', [110 120 60 20]);
