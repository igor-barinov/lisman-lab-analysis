...
n_thresholds; % run the gui and return preset or altered values
filterwindow = n_thresholds_list(1); %setting filter. if =3 makes 3x3 smoothing before calculation-nick
BMF = n_thresholds_list(2); %background multiplication factor: spc_background = BMF*mean+2*std; if BMF=0 then background = low threthould LUT in lifetime map fig3-nick
BMFr = n_thresholds_list(3);% bg=BMFr * mean+2std similar to above but if BMF =0 then red_background = set_red-bg
SD_BMFr = n_thresholds_list(4); % muliplication factor for SD . nomally it is =2
set_red_bg = n_thresholds_list(5);% if 0 then bg = mean + 2 SD- nicko
% pixelshift=0;
sdf = n_thresholds_list(6);
rdf = n_thresholds_list(7);
Red = n_thresholds_list(8); Green = thresholds_list(9); Yellow = thresholds_list(10); % red image mask  color
deltaX = n_thresholds_list(11); %these are ROI shift corection for the red ROI relatively spc-Roi-nicko
deltaY = n_thresholds_list(12);
set_nRoi = n_thresholds_list(13);% if 0 nRoi are automatic otherwise set nRoi including bg ROi
plotave = n_thresholds_list(14);%-nicko 
page3 = n_thresholds_list(15);%if page3=1 then time1=time3 otherwise time1=time-nicko
dpixels = n_thresholds_list(16);%   Make dpixels=1 if dendpixels are calculated in the last region-nicko
...
set_green_bg = n_thresholds_list(19);% if 0 then bg = mean + 2 SD- nicko
% pixelshift=0;
...
nChannels = spc.SPCdata.scan_rout_x; %RY
...
for j = 1:nRoi
    if ishandle(gui.spc.figure.roiB(j));
        ...
        elseif strcmp(get(gui.spc.figure.roiB(j), 'Type'), 'line')% for polygonal ROI -nicko
            ...
            ROI = [xi(:), yi(:)];greenROIreg = roipoly(ones(siz(2), siz(3)),xi+deltaX, yi+deltaY);% corrects position of red ROI. Use only if NEEDED!!!-nicko
%             redROI = [xi+deltaX(:), yi+deltaY(:)];%        ;% corrects position of red ROI. Use only if NEEDED!!!-nicko
        else
            ROI = 0;
            ...
            if j == 1  % background ROI 
               if ~spc.switches.noSPC  %SPC image background ROI statistics
                ...
                if BMF == 0
                  spc_bg =spc.switches.lutlim(1); %avg_bg_intensity+4*ab_std %NickO
                end
               end 
...
%                      end
                ...
                if ~spc.switches.noSPC 
                  mTextBox = uicontrol(thresholds, 'style','edit','Position', [70 40 60 20]);
                  ...
                  mTextBox2 = uicontrol(thresholds, 'style','edit','Position', [230 40 60 20]);
                  set(mTextBox2,'string',red_bg_mean); 
                  mTextBox3 = uicontrol(thresholds, 'style','edit', 'Position', [230 80 60 20]);
                  set(mTextBox3,'string',red_bg); %red_bg_mean +2*red_ab_std
                end  
               if j==nRoi
                   if ~spc.switches.noSPC 
                    mTextBox4 = uicontrol(thresholds, 'style','edit','Position', [70 120 60 20]);
                    set(mTextBox4,'string',spine_spc_max/sdf); %+2*red_ab_std
                   else 
                    mTextBox5 = uicontrol(thresholds, 'style','edit', 'Position', [230 120 60 20]);
    ...
    figure('Name','Red','NumberTitle','off'); %nick
...
%      fig_content = {'tau_m', 'int_int2', 'dendpixels'};
    ...
    fig_content = {'ratio', 'green_mean', 'red_mean'};
    fig_yTitle = {'Ratio', 'green_mean', 'red_mean'};    
end
