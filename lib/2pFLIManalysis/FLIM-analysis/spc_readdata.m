...
if (spc.switches.imagemode == 1)
    spc.imageMod = reshape(spc.image, spc.size(1), spc.size(2), spc.size(3));
    %spc.imageMod = spc.image;
    spc.imageMod = reshape(spc.imageMod, spc.size(1), spc.size(2), spc.size(3));
...
spc.SPCdata.scan_size_y = spc.datainfo.scan_y;
