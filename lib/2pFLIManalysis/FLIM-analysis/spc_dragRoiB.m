...
if strcmp (get(gcf, 'SelectionType'), 'normal')
    for i = 1:length(Rois)
        if strcmp(get(gui.spc.figure.roiB(roiN), 'Type'), 'line')
            set(Rois(i), 'XData', RoiRectX + diffx);
            set(Rois(i), 'YData', RoiRectY - diffy);
            set(Texts(i), 'Position', [RoiRectX(1) + diffx - 2, RoiRectY(1) - diffy -2]);
            xi = get(gui.spc.figure.roiA(roiN), 'XData');
            yi = get(gui.spc.figure.roiA(roiN), 'YData');
            spc.fit.spc_roi{roiN} = [xi(:), yi(:)];
        else
