        ...
        for i = 1:nRoi
            if ishandle(gui.spc.figure.roiB(i))
                if strcmp(get(gui.spc.figure.roiB(i), 'Type'), 'rectangle')
                    ...
                    saveRoi = spc.fit.spc_roi{i};
                    spc.fit.spc_roi{i}(1:2) = spc.fit.spc_roi{i}(1:2) + shift;
                    if spc.fit.spc_roi{i}(1) < 0 || spc.fit.spc_roi{i}(2) < 0 ...
                            || spc.fit.spc_roi{i}(1)+spc.fit.spc_roi{i}(3) > size(spc.project, 1) ...
                            || spc.fit.spc_roi{i}(2)+spc.fit.spc_roi{i}(4) > size(spc.project, 2)
                        spc.fit.spc_roi{i} = saveRoi;
                    end
