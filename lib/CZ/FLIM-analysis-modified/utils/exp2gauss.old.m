...
%{
% ********* Double Exponential calculation ************
decay_param1 = (tau_g^2 * tau1^2)/2 - tau1*(x - tau_d);
decay_param2 = (tau_g^2 * tau2^2)/2 - tau2*(x - tau_d);
pulse_param1 = (tau_g^2 * tau1 - (x - tau_d)) / (tau_g*sqrt(2));
pulse_param2 = (tau_g^2 * tau2 - (x - tau_d)) / (tau_g*sqrt(2));
decay1 = exp(decay_param1);
decay2 = exp(decay_param2);
pulse1 = erfc(pulse_param1);
pulse2 = erfc(pulse_param2);
exp1 = decay1.*pulse1*beta0(1)/2;
exp2 = decay2.*pulse2*beta0(3)/2;
y = exp1 + exp2;
%}
